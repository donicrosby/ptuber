...
impl UserInputMonitor {
    ...
    fn get_gamepad_events(gilrs: &mut Gilrs, joystick_id: usize, mouse_stick: &GamepadMouseStick) -> (Vec<DeviceEvent>, Vec<KeyboardEvent>){
        let mut device_events = Vec::new();
        let mut keyboard_events = Vec::new();
        while let Some(Event {id, event, ..}) = gilrs.next_event() {

            if joystick_id == id.into() {
                match event {
    ...
    pub fn get_events(&mut self) {
        ...
        if let (Some(gilrs), Some(joystick_id)) = (self.gilrs.as_mut(), self.joystick.as_ref()) {
            (gamepad_device_events, gamepad_keyboard_events) = Self::get_gamepad_events(gilrs, *joystick_id, &self.mouse_stick);
